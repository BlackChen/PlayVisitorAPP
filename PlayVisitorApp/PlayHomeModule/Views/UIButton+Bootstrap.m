...
- (void)setButtonIcon:(FAIcon)icon Title:(NSString *)title beforeTitle:(BOOL)before{
     NSString *iconString = [NSString stringFromAwesomeIcon:icon];
    self.titleLabel.font = [UIFont fontWithName:@"FontAwesome"
                                           size:self.titleLabel.font.pointSize];
    NSString *titlee = [NSString stringWithFormat:@"%@", iconString];
    
    if(self.titleLabel.text) {
        if(before)
            titlee = [titlee stringByAppendingFormat:@" %@", title];
        else
            titlee = [NSString stringWithFormat:@"%@  %@", title, iconString];
    }
    
    [self setTitle:titlee forState:UIControlStateNormal];
}
